...
var sellerRegisterDb = require('./schema/sellerRegistrationSchema');
var sellerOtpDb = require('./schema/sellerOtpSchema')
...
class Service {
    ...
    try {
      ...
      if (emailresult.length !== 0) {
        ...
        mailer({
          ...
          to: result.email,
          subject: 'Verify your account to have access in selling products',
  ...
  async findAvailable(params) {
    try {
      const data = sellerOtpDb.find({ $and: [{ email: params.email }, { status: 'ACTIVE' }] }).sort({ createdAt: -1 }).lean().exec()
      return data
    } catch (error) {
      return error
    }
  ...
  async updateOtp(params) {
    try {
      const result = sellerOtpDb.findOneAndUpdate({ email: { $eq: params.email } }, { $set: { otp: params.otp } }).sort({ createdAt: -1 }).lean().exec()
      return result
    } catch (error) {
      return e
    }
  ...
  async createOtpSend(params) {
    const data = new sellerOtpDb(params)
  ...
  async getRegisterUser(params) {
    try {
      const result = sellerOtpDb.find({ $and: [{ email: params }, { status: 'ACTIVE' }] }).sort({ createdAt: -1 }).lean().exec()
      return result
    } catch (e) {
      return e
    }
  ...
  async createRegisterUser(params) {
    try {
      const id = params
      const result = sellerOtpDb.findOneAndUpdate({ email: { $eq: id } }, { $set: { status: "DELETE" } }).sort({ createdAt: -1 }).lean().exec()
      const result_2 = sellerRegisterDb.findOneAndUpdate({ email: { $eq: id } }, { $set: { status: "ACTIVE" } }).sort({ createdAt: -1 }).lean().exec()
      return result
    } catch (e) {
      return e
    }
  ...
  async getemail(params) {
    try {
      const result = sellerRegisterDb.find({ $and: [{ email: params }, { status: 'ACTIVE' }] }).sort({ createdAt: -1 }).lean().exec()
      return result
    } catch (e) {
      return e
    }
  ...
  async getPhone(params) {
    try {
      const result = sellerRegisterDb.find({ $and: [{ phone: params }, { status: 'ACTIVE' }] }).sort({ createdAt: -1 }).lean().exec()
      return result
    } catch (e) {
      return e
    }
  ...
  async saveData(params) {
    try {
      const data = new sellerRegisterDb(params)
      const check = sellerRegisterDb.find({ $and: [{ email: { $eq: params.email } }, { status: { $eq: 'INACTIVE' } }] }).sort({ createdAt: -1 }).lean().exec()
      return check
    } catch (error) {
      return e
    }
  ...
  async saveDatas(params, datas) {
    try {
      ...
      if ((verify === 0) && (params.password === params.confirmPassword)) {
        return sellerRegisterDb(datas).save()
      } else {
        return datas
      }
    } catch (error) {
  ...
  async getRegisterAccount() {
    try {
      const result = sellerRegisterDb.find({ status: "ACTIVE" }).sort({ createdAt: -1 }).lean().exec()
  ...
  async getRegisterAccountById(param) {
    return sellerRegisterDb.find({ $and: [{ _id: param }, { status: "ACTIVE" }] }).sort({ createdAt: -1 }).lean().catch((e) => {
      return e
    })
  }
  ...
  async updateRegisterAccountById(id, params) {
    return sellerRegisterDb.findOneAndUpdate(id, params).sort({ createdAt: -1 }).lean().catch((e) => {
      return e
    })
  }
  ...
  async deleteRegisterAccountById(id) {
    return sellerRegisterDb.findOneAndUpdate({ _id: { $eq: id } }, { $set: { status: "DELETE" } }).sort({ createdAt: -1 }).lean().catch((e) => {
      return e
    })
  }
  ...
  async getuserLogIn(params) {
    try {
      return sellerRegisterDb.find({ $and: [{ email: params }, { status: 'ACTIVE' }] }).sort({ createdAt: -1 }).lean().exec()
    } catch (error) {
      return error
    }
  }
